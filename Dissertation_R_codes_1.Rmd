...
# install.packages(c("kableExtra", "knitr", "tidyverse", "dplyr", "ggplot2", "kableExtra", "gridExtra", "ggsci", "RColorBrewer", "viridis","lattice", "caret", "vars", "scales", "jsonlite","sf", "stringr", "bestNormalize", "broom", "GGally", "quantreg","patchwork", "evd", "goftest", "ggpubr", "eva", "purrr", "simhelpers", "openxlsx"))
...
EBITDA.tau.box <- ggplot(EBITDA.margin.MC.forecast.results,
...
```{r TAT projection visual 2, fig.width=16, fig.height=6} 
# Boxplot by Scenario, faceted by tau

TAT.tau.box <- ggplot(TAT.MC.forecast.results,
       aes(x = Scenario, y = pred_values, fill = Scenario)) +
  geom_boxplot(outlier.size = 0.2) +
  scale_y_continuous(limits = c(-5, 20)) + 
  scale_fill_bmj() +
...
EBITDA.tau.box + TAT.tau.box + plot_layout(ncol = 2)
```
...
## Store sample output
```{r Store output in Excel}
sample.sim <- sample(runs, 50)

sampled.EBITDA.results <- EBITDA.margin.MC.forecast.results %>%
  filter(Simulation %in% sample.sim)
sampled.TAT.results <- TAT.MC.forecast.results %>%
  filter(Simulation %in% sample.sim)

MC.sampled.output.list <- list(
  "EBITDA_Margin_Results" = sampled.EBITDA.results,
  "TAT_Results" = sampled.TAT.results
)

WorkBook <- createWorkbook()

for (name in names(MC.sampled.output.list)) {
  addWorksheet(WorkBook, sheetName = name)
  writeData(WorkBook, sheet = name, x = MC.sampled.output.list[[name]])
}

saveWorkbook(WorkBook, file = "Monte_Carlo_Forecast_Results_Sample.xlsx", overwrite = TRUE)
```

## Store summary statistics

```{r Excel summary stats}
interested <- c("Composite", "pred_values")
EBITDA.MC.forecast.summary <- EBITDA.margin.MC.forecast.results %>%
  group_by(Activities, Scenario) %>%
  summarise(across(
    all_of(interested),
    list(
      Mean   = ~mean(ifelse(is.na(.x), 0, .x)),
...
addWorksheet(WorkBook, "EBITDA_Forecast_Summary_Stats")
writeData(WorkBook, sheet = "EBITDA_Forecast_Summary_Stats",
          x = EBITDA.MC.forecast.summary)

TAT.MC.forecast.summary <- TAT.MC.forecast.results %>%
  group_by(Activities, Scenario) %>%
  summarise(across(
    all_of(interested),
    list(
...
addWorksheet(WorkBook, "TAT_Forecast_Summary_Stats")
writeData(WorkBook, sheet = "TAT_Forecast_Summary_Stats",
          x = TAT.MC.forecast.summary)
